...
export function toggleBookmarks () {
  ...
  if (window.location.pathname.startsWith("/item")) {
    const button = document.querySelector("#detailsSaveButton")
    const pathParts = window.location.pathname.split("/");
    const id = pathParts[2];

    setEventListener(id, button)
    loadSavedState(id, button)
  } else {
  ...
  // Setting eventlisteners on button
  function setEventListener (id, button) {
  ...
  // loading savedstate
  function loadSavedState(id, button) {
